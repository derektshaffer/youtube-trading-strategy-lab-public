...
- Capture the stock universe, price range, liquidity, relative volume, VWAP, breakout level,
  VWAP hold/reclaim quality, volume acceleration, ATR/volatility, confirmed HH/HL market structure,
  and confirmed-swing breakout/failure conditions when the source states them.
...
def normalize_machine_rules(raw_rules: dict[str, Any] | None) -> dict[str, Any]:
    ...
    integer_fields = {
        ...
        "avwap_pivot_confirm_bars", "avwap_anchor_session_minute", "minimum_vwap_hold_bars",
    }
    boolean_fields = {
    ...
    for name in ("min_atr_pct", "max_atr_pct"):
