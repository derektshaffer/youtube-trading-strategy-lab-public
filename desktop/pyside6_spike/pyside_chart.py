"""Native interactive candlestick chart used by the PySide6 framework spike."""
...
class CandleChart(QWidget):
    ...
    def paintEvent(self, _event: Any) -> None:
        ...
        if not rows:
            ...
            painter.drawText(18, 30, "Waiting for chart data…")
        ...
        for row in rows:
            ...
            if self.show_vwap:
                values.append(float(row["vwap"]))
            if self.show_ema:
                values.append(float(row["ema_9"]))
        minimum = min(values)
        maximum = max(values)
        ...
        for marker in range(markers):
            index = min(
                len(rows) - 1,
                round(marker * (len(rows) - 1) / max(1, markers - 1)),
            )
            stamp = time.strftime(
                "%H:%M",
                time.localtime(int(rows[index]["time"])),
            )
        ...
        def draw_indicator(key: str, color: str) -> None:
            ...
            painter.setPen(pen)
            points = [
                QPointF(x_at(index), y_at(float(row[key])))
