...
class JobStore:
    ...
    def _assert_worker(row: sqlite3.Row, worker_id: str | None) -> None:
        ...
        if not assigned:
            raise JobStateError("Job has no active worker lease.")
    ...
    def transition(
        ...
        *,
        ...
        with self.database.transaction(immediate=True) as connection:
            ...
            if current_state in TERMINAL_JOB_STATES or current_state in {
                JobState.QUEUED,
                JobState.RETRY_WAIT,
            }:
                raise JobStateError(f"Cannot advance a {current_state.value} job.")
            self._assert_worker(row, worker_id)
            ...
            proposed = float(progress if progress is not None else row["progress"] or 0.0)
            next_progress = max(
                float(row["progress"] or 0.0),
                min(0.999, max(0.0, proposed)),
            )
        ...
        *,
        ...
        with self.database.transaction(immediate=True) as connection:
            ...
            if state in {JobState.QUEUED, JobState.RETRY_WAIT}:
                raise JobStateError(f"Cannot fail a {state.value} job without a worker lease.")
            self._assert_worker(row, worker_id)
            can_retry = (
                bool(retryable)
